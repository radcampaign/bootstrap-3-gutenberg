...
// import BlockListBlock from '@wordpress/editor/build-module/components/block-list/block.js';
import BlockListBlock from './block.js';
import { DefaultBlockAppender as BlockListAppender } from 'wp.editor';
import { getBlockDOMNode } from '../../util/dom.js';

class ColumnsBlockList extends Component {
	constructor( props ) {
		super( props );
		...
		this.onPointerMove = _.throttle( this.onPointerMove.bind( this ), 100 );
	...
	onPointerMove( { clientY } ) {
		...
		const key = _.findLast( this.coordMapKeys, ( coordY ) => coordY < y );
	...
	onSelectionStart( clientId ) {
		...
		this.coordMapKeys = _.sortBy( Object.values( clientIdToCoordMap ) );
	...
	render() {
		...
		return el("div", {
        className: "editor-block-list__layout row rb-editor-row"
      },
      _.map(blockClientIds, (clientId, blockIndex) => {
